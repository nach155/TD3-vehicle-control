...
class TD3Agent:
    def __init__(self):
        ...
        # self.NOISE_STDDEV = lambda episode: 0.2 if episode <= self.N_EPISODE/2 else 0.2*(1-episode/self.N_EPISODE)
    ...
    def play(self,n_episode):
        ...
        for n in range(n_episode):
            self.NOISE_STDDEV = (lambda episode: 0.2 if episode <= n_episode/2 else 0.2*(1-(episode-n_episode/2)*2/n_episode))(n)
    ...
    def test_play(self, n, monitordir, load_model=False):
        ...
        for i in range(n):
            ...
            video  = cv2.VideoWriter(f'{monitordir}/Video-{date.today()}-{i+1}.mp4', fourcc, 5, frame.shape[:-1],isColor=True)
            video.write(frame)
            while not done:
                action = self.actor.sample_action(state, noise=self.NOISE_STDDEV)
                next_state, reward, done, _ = env.step(action)
...
def main():
    ...
    agent.test_play(n=10, monitordir="./history/movie", load_model=True)
